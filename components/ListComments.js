  ...
  const deleteComment = async (id) => {
    ...
    try {
      const resp = await axios.post(
        `https://blog-backend-4u64.onrender.com/comment/${id}/delete`,
        {},
        {
          headers: {
      ...
      if (resp) {
        setComments(_comments.filter((t) => t._id !== id));
  ...
  return (
    <div className="flex flex-col cursor-pointer">
      ...
      {_comments.map((comment) => {
        ...
        return (
          <div
            className="flex flex-row gap-56 justify-between border-0 border-solid border-b
