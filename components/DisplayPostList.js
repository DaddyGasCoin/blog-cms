...
export default function DisplayPostList({ posts }) {
    ...
    const deletePost = async (id) => {
        ...
        try {
            ...
            const resp = await axios.post(`https://blogapi-production-d43c.up.railway.app/post/${id}/delete`, {}, {
                headers: {
                ...
                return (
                    <div className="flex flex-row gap-3" key={post._id}>
                        ...
                        <div className="flex gap-1">
                            ...
                            <Button size='small' icon={<DeleteOutlined />} onClick={() => showPromiseConfirm(post._id)} danger />
                        </div>
                    </div>
                )
            })}
